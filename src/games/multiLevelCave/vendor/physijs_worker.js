  ...
  switch (description.type) {
    case "plane":
      ...
      if ((shape = getShapeFromCache(cache_key)) === null) {
        shape = new Ammo.btStaticPlaneShape(
          new Ammo.btVector3(
            description.normal.x,
            description.normal.y,
            description.normal.z
    ...
    case "heightfield":
      ...
      var localScaling = new Ammo.btVector3(
  ...
  switch (params.broadphase.type) {
    case "sweepprune":
      broadphase = new Ammo.btAxisSweep3(
        new Ammo.btVector3(
          params.broadphase.aabbmin.x,
          params.broadphase.aabbmin.y,
          params.broadphase.aabbmax.z
        ),
        new Ammo.btVector3(
          params.broadphase.aabbmax.x,
          params.broadphase.aabbmax.y,
          params.broadphase.aabbmax.z
        )
      );
      break;
...
public_functions.setGravity = function (description) {
  world.setGravity(
    new Ammo.btVector3(description.x, description.y, description.z)
  );
};
...
public_functions.addObject = function (description) {
  ...
  if (description.children) {
    ...
    for (i = 0; i < description.children.length; i++) {
      ...
      trans.setOrigin(
        new Ammo.btVector3(
          _child.position_offset.x,
          _child.position_offset.y,
          _child.position_offset.z
        )
      );
      trans.setRotation(
...
public_functions.addWheel = function (description) {
  if (_vehicles[description.id] !== undefined) {
    ...
    _vehicles[description.id].addWheel(
      new Ammo.btVector3(
        description.connection_point.x,
        description.connection_point.y,
        description.connection_point.z
      ),
      new Ammo.btVector3(
        description.wheel_direction.x,
        description.wheel_direction.y,
        description.wheel_direction.z
      ),
      new Ammo.btVector3(
        description.wheel_axle.x,
        description.wheel_axle.y,
        description.wheel_axle.z
      ),
...
public_functions.updateTransform = function (details) {
  ...
  if (details.pos) {
    _transform.setOrigin(
      new Ammo.btVector3(details.pos.x, details.pos.y, details.pos.z)
    );
  }
...
public_functions.applyCentralImpulse = function (details) {
  _objects[details.id].applyCentralImpulse(
...
public_functions.applyCentralForce = function (details) {
  _objects[details.id].applyCentralForce(
    new Ammo.btVector3(details.x, details.y, details.z)
  );
  _objects[details.id].activate();
};
...
public_functions.conetwist_setMotorTarget = function (params) {
  var constraint = _constraints[params.constraint];
  constraint.setMotorTarget(
    new Ammo.btQuaternion(params.x, params.y, params.z, params.w)
  );
  constraint.getRigidBodyA().activate();
  constraint.getRigidBodyB().activate();
};
...
public_functions.dof_setLinearLowerLimit = function (params) {
  ...
  constraint.setLinearLowerLimit(
    new Ammo.btVector3(params.x, params.y, params.z)
  );
  constraint.getRigidBodyA().activate();
  if (constraint.getRigidBodyB()) {
    constraint.getRigidBodyB().activate();
